...
export const WarnWithCard = (input: string, print: GluegunPrint) => {
   ...
   const inputByParagraph = inputByWords.reduce((acc, curr, index) => {
      let lastPositionAcc = acc[acc.length - 1]
   
      const concatenated = [lastPositionAcc, curr]
   
      const joinedStrings = index === 0 ? concatenated.join('') : concatenated.join(' ')
   ...
   function generateCard(lines) {
      ...
      const HIGHER_LINE_WIDTH = lines.reduce((acc, curr) => curr.length > acc ? curr.length : acc , 0)
   
      const headLine = `${LEFT_TOP_CORNER}${fillWithChar(LINE_CHAR, HIGHER_LINE_WIDTH)}${RIGHT_TOP_CORNER}`
      const tailLine = `${LEFT_BOTTOM_CORNER}${fillWithChar(LINE_CHAR, HIGHER_LINE_WIDTH)}${RIGHT_BOTTOM_CORNER}`
   
      const generatedLines = lines.map(line => `│   ${padding(HIGHER_LINE_WIDTH, line)}   │`)
      const placeholderLine = `│   ${padding(HIGHER_LINE_WIDTH, '')}   │`
   
      return card.concat([' ', headLine, placeholderLine], generatedLines, [placeholderLine, tailLine, ' ']).join('\n')
   }
